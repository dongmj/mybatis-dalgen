    ...
    void setCfOperationPageCdata(String cdata, CfOperation cfOperation) {
        ...
        if (cfOperation.getMultiplicity() == MultiplicityEnum.paging) {//分页配置
            ...
            if(selectFromMather.find()){
                Matcher groupByMatcher = GROUP_BY_PATTERN.matcher(cdata);
                if(groupByMatcher.find()) {
                	String temp = groupByMatcher.replaceFirst(REPLACE_TMP);
                	int groupByIndexof = temp.indexOf(REPLACE_TMP);
                	temp = temp.replaceAll(HAVING_PATTERN_STR, REPLACE_TMP);
                	temp = temp.replaceAll(ORDER_BY_PATTERN_STR, REPLACE_TMP);
                    int otherIndexOf = temp.indexOf(REPLACE_TMP, groupByIndexof + 11);
                    String groupByColumns = "";
                    if(otherIndexOf > 0)
                    	groupByColumns = temp.substring(groupByIndexof + 11, otherIndexOf);
                    else
                    	groupByColumns = temp.substring(groupByIndexof + 11);
                    forCount = selectFromMather.replaceFirst("SELECT\n          COUNT(DISTINCT " + groupByColumns + ") AS total \n        FROM\n");
                } else {
                	forCount = selectFromMather.replaceFirst("SELECT\n          COUNT(*) AS total \n        FROM\n");
                }
            }
